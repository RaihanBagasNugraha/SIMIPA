...
class Ta_model extends CI_Model
{
	...
	{
		$query = $this->db->query("SELECT COUNT(*) AS 'jml' FROM seminar_sidang,tbl_users_mahasiswa,tbl_users_dosen,tugas_akhir WHERE tbl_users_dosen.id_user = $id_user AND tbl_users_dosen.jurusan = tbl_users_mahasiswa.jurusan AND tbl_users_mahasiswa.npm = tugas_akhir.npm AND tugas_akhir.status = 4 AND tugas_akhir.npm LIKE '$angkatan%' AND (tugas_akhir.npm LIKE '__$npm1%' OR tugas_akhir.npm LIKE '__$npm2%') AND tugas_akhir.id_pengajuan = seminar_sidang.id_tugas_akhir AND seminar_sidang.jenis = '$seminar' AND (seminar_sidang.status != 6 AND seminar_sidang.status != 5 and seminar_sidang.status >= 7)");
		return $query->row();
	}

	function get_seminar_rekap_koor_detail($id_user,$angkatan,$npm1,$npm2,$seminar,$jenis)
	{
		$query = $this->db->query("SELECT * FROM seminar_sidang,tbl_users_mahasiswa,tbl_users_dosen,tugas_akhir WHERE tbl_users_dosen.id_user = $id_user AND tbl_users_dosen.jurusan = tbl_users_mahasiswa.jurusan AND tbl_users_mahasiswa.npm = tugas_akhir.npm AND tugas_akhir.status = 4 AND tugas_akhir.npm LIKE '$angkatan%' AND (tugas_akhir.npm LIKE '__$npm1%' OR tugas_akhir.npm LIKE '__$npm2%') AND tugas_akhir.jenis = '$jenis' AND tugas_akhir.id_pengajuan = seminar_sidang.id_tugas_akhir AND seminar_sidang.jenis = '$seminar' AND (seminar_sidang.status != 6 AND seminar_sidang.status != 5 and seminar_sidang.status >= 7) ");
		return $query->result();
	}

	function get_mahasiswa_ta_rekap_jml($id_user,$angkatan,$npm1,$npm2)
	{
		$query = $this->db->query("SELECT count(DISTINCT tugas_akhir.npm) as jml FROM tugas_akhir, tbl_users_mahasiswa,tbl_users_dosen,seminar_sidang, seminar_sidang_kompre WHERE tbl_users_dosen.id_user = $id_user AND tbl_users_mahasiswa.jurusan = tbl_users_dosen.jurusan AND tugas_akhir.npm = tbl_users_mahasiswa.npm AND tugas_akhir.npm LIKE '$angkatan%' AND (tugas_akhir.npm LIKE '__$npm1%' OR tugas_akhir.npm LIKE '__$npm2%' ) AND tugas_akhir.id_pengajuan = seminar_sidang.id_tugas_akhir AND seminar_sidang.status > 6 AND seminar_sidang.id != seminar_sidang_kompre.id_seminar");
		return $query->row();
	}

	function get_mahasiswa_ta_rekap_lulus($id_user,$angkatan,$npm1,$npm2)
	{
		$query = $this->db->query("SELECT count(DISTINCT tugas_akhir.npm) as jml FROM tugas_akhir, tbl_users_mahasiswa,tbl_users_dosen,seminar_sidang, seminar_sidang_kompre WHERE tbl_users_dosen.id_user = $id_user AND tbl_users_mahasiswa.jurusan = tbl_users_dosen.jurusan AND tugas_akhir.npm = tbl_users_mahasiswa.npm AND tugas_akhir.npm LIKE '$angkatan%' AND (tugas_akhir.npm LIKE '__$npm1%' OR tugas_akhir.npm LIKE '__$npm2%' ) AND tugas_akhir.id_pengajuan = seminar_sidang.id_tugas_akhir AND seminar_sidang.status > 6 AND seminar_sidang.id = seminar_sidang_kompre.id_seminar AND seminar_sidang_kompre.ket = 1 AND seminar_sidang_kompre.status = 1");
		return $query->row();
	}
